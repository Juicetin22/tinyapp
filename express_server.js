...
app.get("/", (req, res) => {
  ...
  if (!users[req.session.user_id]) {
    res.redirect("/login");
  //if they are then redirect them to their list of URLs page
  } else {
    res.redirect("/urls");
  }
});
...
app.post("/urls", (req, res) => {
  ...
  //prevent users who are not logged in from creating a new shortURL
  if (!users[req.session.user_id]) {
    res.status(401).send("<h2>Cannot create new URLs when not logged in. Please login!</h2>");
  } else {
    //generate the random ID for the new shortURL using a helper function
...
app.post("/urls/:shortURL", (req, res) => {
  ...
  //if user is not stopped by the two above conditions, then the person accessing the URL is the owner and can edit the URL
  } else {
...
app.get("/u/:shortURL", (req, res) => {
  ...
  //if the shortURL is not in the database, send the user an error page
  if (!urlDatabase[req.params.shortURL]) {
...
app.post("/urls/:shortURL/delete", (req, res) => {
  ...
  // if user is not stopped by the two checks above, allow the deletion of the URL
  } else {
...
app.post("/login", (req, res) => {
  //store login info from submit form in variables
  const loginUserEmail = req.body.email;
  const loginUserPassword = req.body.password;
  //checks for a user in the database by their email using a helper function
  ...
  //if the password inputted does not correspond to the password in the database, return error message
  ...
  //if both checks pass, then set user to an encrypted cookie using their id and redirect them to their main URL page
  } else {
...
app.post("/register", (req, res) => {
  //generate user id with helper function, which will be encrypted as a cookie when logging in
  const id = generateRandomString();
  //store submit form info in variables
  const email = req.body.email;
  const password = req.body.password;
  //encrypt password with bcrypt
  const hashedPassword = bcrypt.hashSync(password, 10);
  //checks for user in database using the email submitted with a helper function; returns user info
  ...
  //if email submitted is already in the database, return an error
  ...
  //if both these checks pass, register the user info in the database, log user in with an encrypted cookie, and redirect to their main URL page
  } else {
    users[id] = {
      id,
      email,
